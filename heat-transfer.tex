...
\item[Overall heat transfer coefficient]
  \(\frac{1}{UA} = \frac{1}{{(\eta_0 h A)}_\mathrm{c}} + \frac{R_\mathrm{f,c}''}{{(\eta_0 A)}_\mathrm{c}}
  + R_\mathrm{w} + \frac{R_\mathrm{f,h}''}{{(\eta_0 A)}_\mathrm{h}} + \frac{1}{{(\eta_0 h A)}_\mathrm{c}}\)
