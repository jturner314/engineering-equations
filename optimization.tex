...
\section{Necessary and Sufficient Conditions for Optimization}\label{sec:necess-suff-conditions}
...
\begin{enumerate*}
\item \label{itm:choose-init-and-step} Choose \(x^0\) and a step size \(\Delta\). Set \(k = 0\).
...
\subsection{Three-Point Quadratic Non-Derivative Optimization with Refinement}\label{sec:quadratic-interpolation}
...
\subsection{Newton--Raphson Method}\label{sec:newton-raphson}

Select a value \(x^0\) and build a linear approximation of \(f'\):
...
\subsection{Two-Point Cubic Optimization with Refinement}\label{sec:cubic-interpolation}

This interpolation-based method uses first derivative information to generate splines for
\(\tilde{f}(x)\) and \(\tilde{f}'(x)\).

\subsection{Interval Halving Region Elimination Method}\label{sec:interval-halving}
...
\subsection{Golden Section Region Elimination Method}\label{sec:golden-section-region}
...
\subsection{Bisection Region Elimination Method}\label{sec:bisection-region}
