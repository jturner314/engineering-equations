...
individual methods determine the direction \(\vec{S}^q\) for each search. This is typically written
as the following for iteration \(q\):
...
\begin{enumerate*}
\item Pick a base point \(\vec{x}^0\).
...
\begin{enumerate*}
\item Pick a starting point \(\vec{x}^0\).
